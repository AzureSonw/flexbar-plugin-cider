...
- Now Playing 歌手下方增加细线播放进度条，仅供显示。
...
保持 Cider 运行并启用本地 API。Now Playing 约每秒通过 v2 接口读取播放时间；歌曲元数据约每三秒或切歌时刷新，其间复用元数据和封面缓存。
...
Now Playing 使用按键加载时收到的宽度。常规封面为 58×58，位于 (1,1)，在 60 像素高的画布中保留 1 像素黑边；非正方形图片居中裁切，不拉伸。歌名和歌手沿用 1.2.1 的水平居中位置及文字宽度。极窄按键或较小的旧图标尺寸会限制封面大小，防止进入原文字区域。FlexDesigner 2.2.3 实测加载事件中的 `key.width` 与 `key.style.width` 相同，因此保留优先使用运行时 `key.width`、缺失时使用 `style.width` 的逻辑。修改宽度后，请应用修改并上传布局，使按键重新加载。SDK 没有文档化的普通按键尺寸变化事件；后续重绘使用最近一次加载的尺寸。

进度条位于 y=50，高 2 像素，深灰色轨道上显示白色已播放部分。起点在实际封面右边缘后 30 像素，终点在画布右边缘前 30 像素，长度随加载的按键宽度变化。歌名和歌手的垂直居中锚点上移至 y=15、y=34；无歌手时歌名为 y=26。时间无效或空间不足时隐藏进度条。暂停后不做本地时间递增，相同画面跳过重绘。点击 Now Playing 任意区域仍切换播放/暂停，进度条不能拖动或跳转，也没有新增按键或原生滑块。
