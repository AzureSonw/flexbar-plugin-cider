...
进度条位于 y=50，高 2 像素，深灰色轨道上显示白色已播放部分。起点在实际封面右边缘后 45 像素，终点在画布右边缘前 45 像素，长度随加载的按键宽度变化。歌名和歌手的垂直居中锚点上移至 y=15、y=34；无歌手时歌名为 y=26。时间无效或空间不足时隐藏进度条。暂停后不做本地时间递增，相同画面跳过重绘。点击 Now Playing 任意区域仍切换播放/暂停，进度条不能拖动或跳转，也没有新增按键或原生滑块。
